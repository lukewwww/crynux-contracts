...
async function getConfiguredPrivateKey(networkName: 'ethereum', accountIndex: number, accountDescription: string): Promise<Hex> {
...
export function getConfiguredDeployerPrivateKey(): Promise<Hex> {
  return getConfiguredPrivateKey('ethereum', 0, 'a deployer account');
}

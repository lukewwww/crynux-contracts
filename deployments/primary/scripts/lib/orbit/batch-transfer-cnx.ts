...
import { orbitRuntime, deploymentConfig, deploymentContracts, getDeployerAccount, orbitChain, orbitChainPublicClient, orbitChainRpcUrl } from './runtime.js';
...
const [csvPathInput, ...extraArgs] = expectAtLeastPositionalArgs(
  0,
  `npx tsx deployments/primary/scripts/${orbitRuntime.scriptDir}/batch-transfer-cnx.ts [csvPath] [--force]`,
  ['--force'],
);
...
if (extraArgs.length > 0) {
  throw new Error(
    `Usage: npx tsx deployments/primary/scripts/${orbitRuntime.scriptDir}/batch-transfer-cnx.ts [csvPath] [--force] --network=<testnet|mainnet>`,
  );
}

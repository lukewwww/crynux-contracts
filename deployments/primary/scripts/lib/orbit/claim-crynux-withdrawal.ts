...
import { orbitRuntime, createOrbitChainProvider, createParentChainProvider, deploymentConfig, getConfiguredDeployerPrivateKey } from './runtime.js';
import { expectPositionalArgs, primaryRuntime } from '../../common.js';

const [withdrawalTxHash] = expectPositionalArgs(
  1,
  `npx tsx deployments/primary/scripts/${orbitRuntime.scriptDir}/claim-crynux-withdrawal.ts <withdrawalTxHash>`,
);
...
console.log(`${deploymentConfig.name} withdrawal status:`);
console.log(
  JSON.stringify(
    {
      withdrawalTxHash,
    ...
    JSON.stringify(
      {
        ...
        status: statusName,
        readyToExecute: true,
        action: `Executing withdrawal on ${orbitRuntime.parentName}.`,
      },
      null,
      2,
    ),

...
import { orbitRuntime, getDeployerAccount, minL2BaseFee, orbitChainPublicClient } from './runtime.js';

const [targetMinL2BaseFeeInput, transactionMaxFeePerGasInput, ...extraArgs] = expectAtLeastPositionalArgs(
  0,
  `npx tsx deployments/primary/scripts/${orbitRuntime.scriptDir}/set-min-l2-base-fee.ts [targetMinL2BaseFeeWei] [transactionMaxFeePerGasWei]`,
);

if (extraArgs.length > 0) {
  throw new Error(
    `Usage: npx tsx deployments/primary/scripts/${orbitRuntime.scriptDir}/set-min-l2-base-fee.ts [targetMinL2BaseFeeWei] [transactionMaxFeePerGasWei] --network=<testnet|mainnet>`,
  );
}

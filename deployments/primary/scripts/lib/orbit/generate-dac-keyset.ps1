param(
    ...
    [string]$Network,
    [Parameter(Mandatory = $true)]
    [string]$TestnetLayerFolder,
    [Parameter(Mandatory = $true)]
    [string]$MainnetLayerFolder,
    [Parameter(Mandatory = $true)]
...
$networkFolder = if ($Network -eq "testnet") { "testnet\$TestnetLayerFolder" } else { "mainnet\$MainnetLayerFolder" }
$targetDir = Join-Path $primaryDir $networkFolder
$configScript = Join-Path $primaryDir "scripts\$CommandDirectory\generate-dac-keyset-config.ts"

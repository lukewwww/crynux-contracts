...
function getTransferArgs(): { recipient: Address; amount: bigint } {
  ...
  if (process.argv.length !== 4 || recipientArg === undefined || amountArg === undefined) {
    throw new Error('Usage: npx tsx deployments/primary/testnet/base-sepolia/transfer-cnx.ts <address> <integer-cnx-amount>');
  }
...
if (account === undefined) {
  throw new Error('Base Sepolia deployer account is required.');
}
...
if (balance < amount) {
  throw new Error(`Insufficient Base Sepolia CNX balance. Required ${formatUnits(amount, 18)}, available ${formatUnits(balance, 18)}.`);
}
...
console.log('Base Sepolia CNX transfer transaction receipt:');
...
console.log(`Base Sepolia CNX balance for ${recipient}: ${formatUnits(recipientBalance, bridgedCrynuxToken.decimals)}`);
